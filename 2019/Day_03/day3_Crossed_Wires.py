...
pygame.init()
window = pygame.display.set_mode((15000, 15000))
rx = 7500
inter_color = (255, 255, 0)
...
def wire_path(wire):
    def go_direction(left, right):
        if (left, right) in path:
            changeColor()
            pygame.draw.circle(window, inter_color, (left+rx, right+rx), 5)
        else:
            path[(left, right)] = count
        pygame.draw.circle(window, color, (left+rx, right+rx), 2)
    ...
    for direction in wire:
        ...
        if direction[0] == 'R':
            for num in range(y+1, y+number+1):
                count += 1
                go_direction(x, num)
            y += number
        elif direction[0] == 'L':
            for num in range(y-1, y-number-1, -1):
                count += 1
                go_direction(x, num)
            y -= number
        elif direction[0] == 'U':
            for num in range(x-1, x-number-1, -1):
                count += 1
                go_direction(num, y)
            x -= number
        elif direction[0] == 'D':
            for num in range(x+1, x+number+1):
                count += 1
                go_direction(num, y)
            x += number
    return path
...
def find_crossings(paths):
    crossings_temp = dict()
    for point, path in paths[0].items():
        if point in paths[1]:
            crossings_temp[point] = path + paths[1][point]
    return crossings_temp


paths = []
for wire in wires:
    paths.append(wire_path(wire))

crossings = find_crossings(paths)
smallest = None
euclidian = None
crossy = None
for cross in crossings:
    pygame.draw.circle(window, (0, 255, 0), (cross[0]+rx, cross[1]+rx), 10)
    num = sum((abs(cross[0]), abs(cross[1])))
    if not smallest:
        smallest = crossings[cross]
    elif crossings[cross] < smallest:
        smallest = crossings[cross]
    if not euclidian:
        euclidian = num
    else:
        if num < euclidian:
            euclidian = num
            crossy = cross
pygame.draw.circle(window, (255, 255, 255), (crossy[0]+rx, crossy[1]+rx), 15)
pygame.display.update()
pygame.image.save(window, 'Advent Of Code\AOC_2019\day3 wire crossings.png')
print(smallest)
print(euclidian)
